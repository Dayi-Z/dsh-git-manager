...
- **横向分隔线要数。** 顶部曾经三条发丝线（头部/流程/页签）；流程条并进头部、去掉它下面那条，只剩两条。装完的仓库（没有 phase===1 的步骤）流程条整条不渲染，面板直接回到"头部 + 页签"两行。
- **内聚的页签合并。** 7 个顶级页签里有三个都属于 GitHub 这一件事（PR / 议题 / 连接），并排占位只会让每个都变窄。收进一个 GitHub 页签 + 分段控件（PR / 议题 / 连接），顶级页签 7 → 5，每个都宽裕。
- **列表的左侧导轨。** 状态字母放最右边时，它和文件名之间是一大片空白，行看着又长又散；挪到最左边定宽排列，整列字母竖着扫一眼就知道哪些是新文件。
- **行高由内容决定，别让悬停控件撑行。** 行内操作钮默认 26px，把 min-height:24px 的行实际撑到 30px（量出来的）。行内操作统一 20px，24px 才真的成立（76 行省下约 450px）。
